...
const Compare: FunctionComponent<Props> = ({ data, country }) => {
	...
	const match = data.orderedBonusList.filter(
		list => list.supported_country.code === country
	)[0]
	if (match) bonuses = match.bonusList
	else {
		bonuses = data.orderedBonusList.filter(
			list => list.supported_country.code === 'row'
		)[0].bonusList
		remappedCountry = 'row'
	}
	...
	return (
		<Wrapper>
			<Container>
				...
				<BonusStripe
					eng={true}
					bonus={bonuses[0].bonus}
					countryCode={remappedCountry}
				/>
